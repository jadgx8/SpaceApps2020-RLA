...
#------------------- Parametros Globales con que se estará manejando el script ---------------------------------------
trainX_lat = list()
testX_lat = list()
trainY_lat = list()
testY_lat = list()
...
def read_csv2(filename):
    ...
    with open(filename, 'rb') as f:
        ...
        for i in range(len(reader)):
            if i > 0:
                ...
                if (i%ave_points == 0.0):
                    trainlistX_lon.append(sum_lon/ave_points)
                    trainlistX_lat.append(sum_lat/ave_points)
    ...
    print ("longitudes de listas medias: ", len(trainlistX_lat), len(trainlistY_lat))
    print ("longitudes de listas medias: ", len(trainlistX_lon), len(trainlistY_lon))
...
print ("Antes de separar la data", trainX_lat.shape, trainY_lat.shape)
type1_split()
print ("Despues de separar la data", trainX_lat.shape, trainY_lat.shape)
...
print ("forma real del modelo después de la remodelación de la data: " , trainX_lat.shape, testX_lat.shape)
...
plt.figure(1)
ax = plt.subplot(211)
loss1 = plt.plot(range(len(hist_lon_dict['val_loss'])), hist_lon_dict['val_loss'], 'r', label='Perdidas en la validación')
loss2 = plt.plot(range(len(hist_lon_dict['loss'])), hist_lon_dict['loss'], 'g', label='Perdidas del entrenamiento')
ax.set_xlabel('Epoch')
ax.set_ylabel('Loss')
plt.legend(handles=[loss1, loss2])
ax.set_title('Curva de perdida del modelo de longitud')
...
ax.set_ylabel('Loss')
plt.legend(handles=[loss1, loss2])
ax.set_title('Curva de pérdida del modelo de latitud')
